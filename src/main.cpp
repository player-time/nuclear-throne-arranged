...
void enemy_die(int ENEMY, int PROJ) {
    ...
    case bandit:
        ...
        if (rand() % 20 == 0) {
            create_object(allObjects[ENEMY].position.x, allObjects[ENEMY].position.y, 0, 0, weapon_drop, 0, rand() % 86);
        }
...
void do_object_logic(int start, int end, sf::SoundBuffer all_sounds[], sf::Music& current_music) {    //logic done on each obect every frame
    ...
    for (int i = start; i < end; i++) {
        ...
        case rad:
            ...
            if (allObjects[i].next_hurt == current_frame || !player_alive) {
...
void player_shell_burst_step(int obj) {
    ...
    if (allObjects[obj].image_index % allObjects[obj].alarm1 == 0) {
        if (allObjects[obj].damage == 3) {//wave
            ...
            inaccuracy = sin(allObjects[obj].size) * 20;
            inaccuracy /= degreestoradians;
...
void player_bolt_step(int obj) {
    ...
    if (allObjects[obj].damage == 9) {  //seeker
        ...
        if (closest_enemy != 0) {
            ...
            motion_add_dir(tmpdir, 0.5f, obj);
...
int main()
{
    ...
    while (window.isOpen()){
        ...
        sf::Text tO;
        tO.setString("DEBUG_ENEMY_TELEPORT_TO X: " + std::to_string(allObjects[DEBUG_ENEMY_TELEPORT_TO].position.x));
